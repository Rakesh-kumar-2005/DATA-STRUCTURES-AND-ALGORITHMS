...
/*
    ...
    Description:
      Following program finds the length of the Longest Increasing Subsequence (LIS)
        of an array using six approaches from pure recursion to binary search O(n log n)...

    Problem Statement:
      -> Given an integer array arr...
      -> A subsequence is strictly increasing if each element is greater than the previous...
      -> Characters need not be contiguous but must maintain original relative order...
      -> Return the length of the longest such strictly increasing subsequence...
      -> Every single element is trivially an LIS of length 1...

    Key Insight:
      -> At each element (curr), two decisions exist:
           Skip it: LIS length unchanged, move to curr+1...
           Pick it: only allowed if arr[curr] > arr[prev], extends LIS by 1...
      -> State = (curr index, prev index) where prev is the index of the last picked element...
      -> prev = -1 initially (no element picked yet, any first element qualifies)...
      -> Overlapping subproblems → DP is optimal...

    Example:
      -> arr = [10, 9, 2, 5, 3, 7, 101, 18]:
           LIS = [2, 3, 7, 101] or [2, 5, 7, 101] → length 4...
      -> arr = [1, 2, 3, 4, 5]:
           Entire array → length 5...
      -> arr = [5, 4, 3, 2, 1]:
           No pair is increasing → single element → length 1...
      -> arr = [7, 7, 7, 7]:
           Strictly increasing: 7 == 7 fails → length 1...

    Recursive Relation:
      -> Base: curr >= n → return 0...
      -> notPick = recursive(curr+1, prev, arr)...
      -> pick = 0...
    ...
    Approach 1 - Recursive:
      -> State: (curr, prev) with prev = -1 initially...
      -> Two branches per element → exponential recomputation...
      -> Time: O(2^n), Space: O(n) recursion stack...

    Approach 2 - Memoization (Top-Down DP):
      -> 2D dp array of size n × (n+1)...
      -> prev is offset by +1 to allow index -1 → dp[curr][prev+1]...
      -> Check cache before recursing, store result after computing...
      -> Time: O(n²), Space: O(n²) + O(n) recursion stack...

    Approach 3 - Tabulation (Bottom-Up DP):
      -> dp[curr][prev+1] = LIS length from curr with last-picked index = prev...
      -> Fill right to left: curr from n-1 down to 0...
      -> Inner loop: prev from curr-1 down to -1...
      -> Answer: dp[0][0] (start at index 0 with no previous = -1 = index 0 in offset table)...
      -> Time: O(n²), Space: O(n²)...

    Approach 4 - Two-Array Space Optimization:
      -> Replaces 2D dp with two 1D arrays: next[] and curr[]...
      -> next[] represents dp[curr+1], curr[] represents dp[curr]...
      -> For each outer curr, fill curr[] based on next[]...
      -> After each outer iteration: next = curr...
      -> Space reduced from O(n²) to O(n)...

    Approach 5 - Classic O(n²) DP (dp[i] = LIS ending at i):
      -> dp[i] = length of LIS ending exactly at index i...
      -> Initialize all dp[i] = 1 (every element alone is LIS of length 1)...
      -> For each i from 1 to n-1:
           For each prev from 0 to i-1:
             If arr[i] > arr[prev]: dp[i] = Math.max(dp[i], dp[prev] + 1)...
      -> Track global max across all dp[i]...
      -> Simpler recurrence but same O(n²) time...

    Approach 6 - Binary Search O(n log n) (Patience Sorting / Tails Array):
      -> Maintain a dynamic list lis[] where lis[i] = smallest tail element of
           all increasing subsequences of length i+1 seen so far...
      -> For each num in arr:
           Binary search lis for num's insertion position...
           If num is larger than all tails: append to lis (extend LIS)...
           Else: replace lis[pos] with num (minimize tail for future extensions)...
      -> lis.size() at the end = LIS length...
      -> This does NOT reconstruct the actual LIS, only finds its length...

    Why Binary Search Works (Patience Sorting Insight):
      -> lis[] is always maintained in sorted order (each replacement is at most equal)...
      -> Replacing a larger tail with a smaller one enables longer extensions later...
      -> lis[i] = best possible last element for an increasing subsequence of length i+1...
      -> The size of lis is always the current LIS length...

    Step-by-Step Trace Binary Search (arr = [10, 9, 2, 5, 3, 7, 101, 18]):
      -> num=10: lis=[10]...
      -> num=9:  pos=0 → replace: lis=[9]...
      -> num=2:  pos=0 → replace: lis=[2]...
      -> num=5:  pos=1 → append: lis=[2, 5]...
      -> num=3:  pos=1 → replace: lis=[2, 3]...
      -> num=7:  pos=2 → append: lis=[2, 3, 7]...
      -> num=101:pos=3 → append: lis=[2, 3, 7, 101]...
      -> num=18: pos=3 → replace: lis=[2, 3, 7, 18]...
      -> Result: lis.size() = 4...

    prev + 1 Offset Explanation:
      -> prev can be -1 (no previous element)...
      -> Array indices can't be -1 in Java...
      -> Shift all prev values by +1: prev=-1 maps to index 0, prev=0 maps to index 1...
      -> dp[curr][prev+1] safely handles all prev values including -1...

    Edge Cases:
      -> Single element → LIS = 1 always...
      -> All same values → strictly increasing fails → LIS = 1...
      -> Strictly increasing → LIS = n...
      -> Strictly decreasing → LIS = 1...
      -> Zigzag pattern → LIS determined by longest monotonic chain...
    ...
    Comparison of All Approaches:
      ...
      -> Two-Array Optimized: Time O(n²),    Space O(n)   — rolling arrays...
      -> Classic O(n²) DP:    Time O(n²),    Space O(n)   — simpler recurrence...
      -> Binary Search:       Time O(n log n),Space O(n)  — optimal...

    Time and Space Complexity:
      -> Best: O(n log n) time, O(n) space for binary search approach...
      -> DP variants: O(n²) time, O(n) to O(n²) space depending on approach...

    Applications:
      -> Finding longest trend in stock price series...
      -> Sequence alignment in bioinformatics (longest common supersequence)...
      -> Scheduling problems where tasks must be executed in increasing order...
      -> Box stacking and envelope nesting problems (2D LIS variants)...
