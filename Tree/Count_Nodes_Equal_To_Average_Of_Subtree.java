...
/*
    ...
    Description:
      Following program counts the number of tree nodes whose value equals the integer
        average of all values in their own subtree, using post-order traversal...

    Problem Statement:
      -> Given the root of a binary tree...
      -> For each node, compute the integer average (floor division) of all values
           in its subtree (including the node itself)...
      -> Count nodes where node.val == floor(subtree_sum / subtree_size)...
      -> Return the total count...

    Key Insight:
      -> To compute the average at each node without re-traversing, use post-order traversal...
      -> Each recursive call returns (subtree_sum, subtree_size) as a 2-element array...
      -> Parent aggregates both values from left and right children in O(1)...
      -> This avoids redundant traversal and achieves O(n) total time...
      -> ans[0] is used as a mutable counter passed by reference via an int array...

    Example:
      -> Tree: root=4, left=8(children 0,1), right=5(child 6):
           Leaf 0: avg=0/1=0, val=0 → match, ans=1...
           Leaf 1: avg=1/1=1, val=1 → match, ans=2...
           Leaf 6: avg=6/1=6, val=6 → match, ans=3...
           Node 8: sum=8+0+1=9, size=3, avg=9/3=3, val=8 → no match...
           Node 5: sum=5+6=11, size=2, avg=11/2=5, val=5 → match, ans=4...
           Root 4: sum=4+8+0+1+5+6=24, size=6, avg=24/6=4, val=4 → match, ans=5...
           Result: 5...

    Post-Order Traversal Design:
      -> Process left subtree first → get (left_sum, left_size)...
      -> Process right subtree next → get (right_sum, right_size)...
      -> Combine at current node:
           sum  = left_sum + right_sum + root.val...
           size = left_size + right_size + 1...
           avg  = sum / size (integer floor division)...
           If avg == root.val: increment ans[0]...
      -> Return {sum, size} to the caller...

    Leaf Node Special Case:
      -> When root.left == null AND root.right == null: leaf node...
      -> A leaf's subtree contains only itself → avg = root.val / 1 = root.val...
      -> Always satisfies the condition → increment ans[0] immediately...
      -> Return {root.val, 1}...
      -> This avoids the null-child check in the recursive structure...

    Missing Child Handling:
      -> If a child pointer is null, return {0, 0} (new int[2])...
      -> This contributes 0 to sum and 0 to size → no effect on parent computation...
      -> Elegant and avoids separate base case branching for missing children...

    Why ans[] is an int Array Instead of int:
      -> Java passes primitives by value → an int parameter cannot be modified by callee...
      -> Arrays are objects passed by reference → any modification to ans[0] persists...
      -> This pattern emulates pass-by-reference for a mutable counter across recursion...

    Step-by-Step Trace (Tree: 1 → left 2 → left 3):
      -> helper(3): leaf → ans=1, return {3, 1}...
      -> helper(2): left={3,1}, right={0,0}...
           sum=2+3=5, size=2, avg=5/2=2, val=2 → match, ans=2...
           return {5, 2}...
      -> helper(1): left={5,2}, right={0,0}...
           sum=1+5=6, size=3, avg=6/3=2, val=1 → no match...
           return {6, 3}...
      -> Result: ans[0] = 2...

    Integer Division Truncation:
      -> avg = sum / size uses integer floor division in Java...
      -> Example: sum=11, size=2 → avg=5 (not 5.5)...
      -> This matches the problem's specification of "integer average"...
      -> No floating point conversion needed...

    Edge Cases:
      -> Single node → leaf case triggers → always matches → return 1...
      -> All same values → every subtree average equals that value → all nodes match...
      -> Highly imbalanced tree → post-order still visits every node exactly once...
      -> Root with one child → null child handled with {0, 0} → no issue...

    Time and Space Complexity:
      -> Time:  O(n) — each node visited exactly once in post-order traversal...
      -> Space: O(h) — recursion stack depth equals tree height h...
               O(log n) for balanced trees, O(n) worst case for skewed trees...

    Applications:
      -> Subtree aggregation problems in binary tree processing...
      -> Statistical analysis of hierarchical data structures...
      -> Tree-based data validation where node values must satisfy aggregate conditions...
      -> Competitive programming problems involving post-order accumulation patterns...
